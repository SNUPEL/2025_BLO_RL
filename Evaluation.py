...
class Simulate_yard:
    ...
    def Run_simulation(self, simulation_day, scheduling_mode, init_yard=None, init_block=None, batch_step=10):
        ...
        for btch in range(batch_step):
            ...
            if day_reward/simulation_day>best_reward:
                best_reward=day_reward/simulation_day
                best_ra=day_rt/simulation_day
                best_td=day_tardy/simulation_day
                best_ett=day_ett/simulation_day
            ave_reward+=day_reward
            ave_rt+=day_rt
            ave_tardy+=day_tardy
            ave_ett+=day_ett

        return data, reward_list, done_list, prob_list, action_list, ave_reward/simulation_day/batch_step,ave_ett/simulation_day/batch_step,ave_rt/simulation_day/batch_step,ave_tardy/simulation_day/batch_step,best_reward,best_ett,best_ra,best_td
...
if __name__=="__main__":
    ...
    checkpoint = torch.load('Final_GAT_RL.pth', map_location=torch.device('cuda'))  # 파일에서 로드할 경우
    ...
    yard=pd.read_excel('yard_middle.xlsx')
    yard=np.array(yard)[:,1:]
    ...
    checkpoint = torch.load('Scheduling_RL_small.pth', map_location=torch.device('cuda'))  # 파일에서 로드할 경우

    full_state_dict = checkpoint['model_state_dict']
    #filtered_state_dict = OrderedDict({k: v for k, v in full_state_dict.items() if 'Critic_net' not in k})
    Simulation.ppo.load_state_dict(full_state_dict, strict=False)
    ...
    mode_list = ['Random', 'SSPT', 'SET', 'SRT', 'ATCS', 'MDD', 'COVERT','RL_full']
    eval_history=np.zeros((eval_num,9,4))
    for e,k in enumerate(mode_list):

        print(k)

        for j in range(eval_num):
            data, reward_list, done_list, prob_list, action_list, ave_reward,ave_ett,ave_rt,ave_tardy,best_reward,best_ett,best_ra,best_td = Simulation.Run_simulation(
                simulation_day=10, scheduling_mode=k, init_yard=eval_set[j][0].copy(),
                init_block=eval_set[j][1].copy(), batch_step=20)

            eval_history[j,e,0]=ave_reward
            print(ave_reward)
            print(best_reward)
            eval_history[j, e,1] = ave_ett
            eval_history[j, e,2] = ave_tardy
            eval_history[j, e,3] = ave_rt
            if e==7:
                eval_history[j, e+1, 0] = best_reward
                eval_history[j, e+1, 1] = best_ett
                eval_history[j, e+1, 2] = best_td
                eval_history[j, e+1, 3] = best_ra
